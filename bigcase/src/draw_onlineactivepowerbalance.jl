function draw_powerbalance(
		bench_x₀, bench_p₀, bench_pᵨ, bench_pᵩ, bench_seq_sr⁺, bench_seq_sr⁻,
		bench_pss_charge_p⁺, bench_pss_charge_p⁻, bench_su_cost, bench_sd_cost,
		bench_prod_cost, bench_cost_sr⁺, bench_cost_sr⁻, secnairos)
	#! Plots power balance
	selected_scenarios = secnairos
	NS = winds.scenarios_nums
	#! calculate different output of grid-connected generators
	conventional_generators_output = bench_p₀[
		(NG * (selected_scenarios - 1) + 1):(NG * selected_scenarios), :]
	winds_generators_mppt_output = zeros(NW, NT)
	for w in 1:NW
		winds_generators_mppt_output[w, :] = winds.scenarios_curve[w, :] * winds.p_max[w, 1]
	end
	winds_generators_output = winds_generators_mppt_output .- bench_pᵩ[
		(NW * (selected_scenarios - 1) + 1):(NW * selected_scenarios), :]
	load_curve = loads.load_curve
	loadcurtailments = bench_pᵨ[
		(ND * (selected_scenarios - 1) + 1):(ND * selected_scenarios), :]
	bess_discharge_power = bench_pss_charge_p⁻[
		(NC * (selected_scenarios - 1) + 1):(NC * selected_scenarios), :]
	bess_charge_power = bench_pss_charge_p⁺[
		(NC * (selected_scenarios - 1) + 1):(NC * selected_scenarios), :]
