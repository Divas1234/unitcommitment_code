	...
	for n in 1:NN
		...
		while clycing_period <= 100
			tem_fittingparameter_vector = creatfrequencyfittingfunction(
				units, winds, NG, NW, flag_method_type, whitenoise_parameter[n])

			if all(x -> abs(x) <= 10, tem_fittingparameter_vector)
				fittingparameter_vector[n, :] = tem_fittingparameter_vector
				break
			end

			if clycing_period >= 100
				println("too many clycing periods")
			else
				clycing_period += 1
			end
		end
