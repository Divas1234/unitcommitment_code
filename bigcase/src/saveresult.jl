function save_UCresults(x₀, bench_x₀, p₀, pᵨ, pᵩ, seq_sr⁺, seq_sr⁻, pss_charge_p⁺, pss_charge_p⁻, su_cost, sd_cost, prod_cost, cost_sr⁺, cost_sr⁻, bench_p₀, bench_pᵨ, bench_pᵩ, bench_seq_sr⁺, bench_seq_sr⁻, bench_pss_charge_p⁺, bench_pss_charge_p⁻,
	...
	if Sys.isapple()
		tem_file_path = "/Users/yuanyiping/Documents/GitHub/unit_commitment_code/bigcase/output/"
	elseif Sys.iswindows()
		tem_file_path = "C:\\Users\\yyp_uestc\\Downloads\\unitcommitment_code-master\\bigcase\\output\\"
	else
		println("Unknown OS")
	end
	return save(tem_file_path * "mydata_1.jld",
		"x₀", x₀,
		"p₀", p₀,
		"pᵨ", pᵨ,
		"pᵩ", pᵩ,
...
function read_UCresults()
	...
	# filepath = pwd()
	jldopen(tem_file_path * "mydata_1.jld", "w") do file
		write(file, "x₀", x₀)
...
function savebalance_result(bench_p₀, bench_pᵨ, bench_pᵩ, bench_pss_charge_p⁺, bench_pss_charge_p⁻, flag)
	...
	filepath₀ = pwd() * "/bigcase/output/"
	if Sys.iswindows()
		if flag == 1
			filepath = filepath₀ * "/bench/"
		elseif flag == 2
			filepath = filepath₀ * "/pros/"
		else
			flag == 3
			filepath = filepath₀ * "/enhance_pros/"
		end
	elseif Sys.isapple()
		if flag == 1
			filepath = "/Users/yuanyiping/Documents/GitHub/unit_commitment_code/bigcase/output/bench/"
		elseif flag == 2
			filepath = "/Users/yuanyiping/Documents/GitHub/unit_commitment_code/bigcase/output/pros/"
		else
			flag == 3
			filepath = "/Users/yuanyiping/Documents/GitHub/unit_commitment_code/bigcase/output/enhance_pros/"
		end
	else
		println("Unknown OS")
	end
