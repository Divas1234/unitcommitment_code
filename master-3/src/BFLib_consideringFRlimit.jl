...
function trans_fun_1(trans_fun_0, 𝚫f_pre, δp_add, δt, k, flag, Q, temflag)
	...
	if flag == 2
		tem = tem[1] .+
			  rand(Normal(0, Q), length(𝚫f_pre)) * whitenoise_parameter * (2^(flag - 1))
	end
	if flag >= 2
		tem = tem[1] .+
			  rand(Normal(0, Q), length(𝚫f_pre)) * whitenoise_parameter * (2 * (flag - 1))
...
function generate_data(initialize, trans_fun_1, obs_fun_1, tt, flag, symflag, temflag)
	...
	for i in 2:tt
		δfₜ_cur = trans_fun_1(
			trans_fun_0, δf[i - 1, 1], δp_add[i - 1, 1], δt, i, flag, 1, temflag)
		δf[i] = δfₜ_cur[1]
...
function reconditional_likelihoods(
		obs_fun_0, δp_measured, δf_inference, t, flag, R, temflag)
	R = 1
...
function particle_filter(
		...
		reconditional_likelihoods, N, tt, flag, symflag, temflag
)
...
function simulate(
	...
	𝚫f_particle_set, 𝚫f_post = particle_filter(
		...
		sample_Num, N, flag, symflag, temflag)
